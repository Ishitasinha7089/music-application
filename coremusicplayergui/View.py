...
class View:
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                ...
                raise Exception("Sorry! you cannot save or load songs")
        ...
        self.addFavourite.configure(command=self.add_song_to_favourites)
        ...
        self.loadFavourite.configure(command=self.load_songs_from_favourites)
    ...
    def add_song_to_favourites(self):
        fav_song_to_index_tuple=self.playList.curselection()
        try:
            if len(fav_song_to_index_tuple)==0:
                raise NoSongSelectedError("Please select a song to add to favourites")
            song_name=self.playList.get(fav_song_to_index_tuple[0])
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            if result[0].find("No song selected")!=-1:
            ...
            messagebox.showerror("DB error","Sorry1 songs cannot be loaded")
    def remove_song_from_favourites(self):
        ...
        try:
            ...
            if(result.find("deleted")!=-1):
                self.playList.delete(fav_song_to_index_tuple[0])
                if hasattr(self,"sel_song_index_tuple"):                               #if self.isPlaying==True:
                    if(fav_song_to_index_tuple[0]==self.sel_song_index_tuple[0]):
                         self.stop_song()
            messagebox.showinfo("Success!!", result)
